...
function isInsideCircle(circle, point) {
  const res = Math.hypot(circle.center.x - point.x, circle.center.y - point.y);
  return res < circle.radius;
}
...
function getDigitalRoot(num) {
  let tempNum = String(num).split('').reduce((acc, el) => +acc + +el);
  if (tempNum < 10) {
    return tempNum;
  }
  tempNum = getDigitalRoot(tempNum);
